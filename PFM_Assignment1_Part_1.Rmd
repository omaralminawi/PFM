...
check <- dataset %>%
  ...
  summarise(different_gvkeys = list(unique(gvkey)))

firms_to_remove <- unique(c(single_year_firms, firms_with_gaps))

data_final <- dataset %>%
  filter(!gvkey %in% firms_to_remove)
```
...
beneish_data <- data_final %>%
  select(gvkey, conm, sale, cogs, xsga, dp, am, ib, oancf, rect, act, ppent, 
         at,lct, dlc, dltt,cyear, datadate)
...
beneish_data <- beneish_data %>% group_by(gvkey, conm) %>% 
                ...
                mutate(llct = lag(lct, n = 1))

```

We can now calculate the indices needed for the Beneish scores, namely we calculate the Days’ Sales in Receivables Index, Gross Margin Index, Asset Quality Index, Sales Growth Index, Depreciation Index, Sales, General and Administrative expenses Index, Leverage Index, and the Accruals. Additionally we add ROE which we will be using to explore the effect
of the Beneish scores on explaining persistence in ROE.
...
beneish_data$rnoe <- ib / (act + ppent - lct)
beneish_data$lrnoe <- lib/(lact + lppent - llct)
detach(beneish_data)
```
...
beneish_data$levi[is.na(beneish_data$levi) & beneish_data$td == 0] <- 1
...
Because of the removal of the first firm-year for each company, some companies now have only one year of data and some missing values.
```{r}

single_year_firms <- beneish_data %>%
  group_by(gvkey) %>%
  summarize(num_years = n()) %>%
  filter(num_years == 1) %>%
  pull(gvkey)

firms_with_gaps <- beneish_data %>%
  arrange(gvkey, datadate) |> 
  group_by(gvkey) %>%
  filter(!all(diff(cyear) == 1)) %>%
  pull(gvkey)
...
```{r}
attach(beneish_data)
beneish_data$bm <- (-4.84) + (0.92 * dsri) + (0.528 * gmi) + (0.404 * aqi) - (0.892 * sgi) + (0.115 * depi) - (0.172 * sgai) + (4.679 * accruals) - (0.327 * levi)
detach(beneish_data)

suspectmanipulation <- beneish_data %>%
  filter(bm > -1.78)
```

Scaling the bm variable to range between 0 and 1 and calculating decile ranks by year.  

```{r}
beneish_data <- beneish_data %>% group_by(cyear) %>% 
                mutate(deca = ntile(bm, 10)) # Calculate decile ranks by year
beneish_data$decascaled <- (beneish_data$deca-1)/9  # Scale decile ranks to range between 0 and 1
```

Filter the N/A values for the decile ranks and the scaled decile ranks
...
Next we show the explanatory power of the calculated M-score on profitability persistence

```{r}

mdl <- rnoe ~ bm + lrnoe + bm:lroe
mdlb <- rnoe ~ decascaled + lrnoe + decascaled:lroe

posroedata <- beneish_data %>%
  filter(rnoe > 0) |> 
  filter(lrnoe > 0)

posroedata[posroedata == Inf] <- NA
posroedata[posroedata == -Inf] <- NA

posroedata <- na.omit(posroedata)

rslt <- lm(mdl, data = posroedata)
rslt2 <- lm(mdlb, data = posroedata)
summary(rslt)
summary(rslt2)
...
for (year in 2014:2022) {
    ...
    data_subset <- subset(posroedata, cyear == year)

    # Fit the linear model to the subset
    # Replace y, x1, x2, etc., with your actual variable names
    model <- lm(roe ~ bm + lroe + bm:lroe, data = data_subset)

    # Store the model's summary in the results list
    results[[as.character(year)]] <- summary(model)
    r2results[[as.character(year)]] <- summary(model)$r.squared
}
